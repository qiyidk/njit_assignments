...
import njit.qiyi.movieRecommendation.core.service.GetRecommendMovie;
...
public class RecommendMovie {
    ...
    public List<Movie> getRecommendMovies(List<Movie> movieList, int n){
        ...
        for(Movie m:movieList) {
            List<SimilarityEntry> entrys = new GetRecommendMovie().getRecommendMovieByMovie(m);
            for(SimilarityEntry e: entrys) {
                Double d = null;
    ...
    public List<Movie> getRecommendMovies(User user, int n){
        List<SimilarityEntry> entry = new GetRecommendMovie().getRecommendMovieByUser(user);
        List<Movie> list = new ArrayList<Movie>();
        for(int i=0; i<n; i++) {
